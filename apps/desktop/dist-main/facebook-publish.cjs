...
async function publishDirectComposer(page, opts = {}) {
  ...
  const pauseMs = actionPauseMs(opts);
  ...
  await humanPause(page, mark, pauseMs, "trước Đăng");
  ...
  if (!hidden) {
    await humanPause(page, mark, pauseMs, "chờ modal Đăng");
...
function actionPauseMs(opts) {
  if (opts && typeof opts.humanPauseMs === "number") return Math.max(0, opts.humanPauseMs);
  return 10000 + Math.floor(Math.random() * 5001);
}

function reelPauseMs(opts) {
  return actionPauseMs(opts);
}
...
async function publishPageComposer(page, opts = {}) {
  ...
  else await humanPause(page, mark, pauseMs(), "Cài đặt bài viết");
...
async function settlePagePublishAfterClick(page, mark, opts = {}) {
  ...
  const pause = typeof opts.humanPauseMs === "number" ? opts.humanPauseMs : actionPauseMs(opts);
  ...
  const start = Date.now();
  let extraClicks = 0;
  await humanPause(page, mark, pause, "sau Đăng");
  await sleep(pause ? 0 : 400);
